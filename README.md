...
- 歌手頁 `#/artist/<slug>`：Wikipedia 條目與 YouTube Music 頻道連結、總覽圖表（含各專輯／單曲播放數的發行年表折線圖）、專輯牆（點開看曲目）、全部歌曲表（搜尋／排序）、演唱會（巡演名稱、期間、場數、地點、場館，展開看每一場；所有演唱會畫在同一張巡演地圖，可個別顯示／隱藏）、金曲獎入圍與得獎紀錄
...
介面圖示一律用 Google Material Symbols（Rounded），寫法是 `<span class="mi">icon_name</span>`，樣式在 `src/style.css` 的 `.mi`。字型只包含用到的圖示，新增一個圖示要一併加進 `index.html` 字型網址的 `icon_names` 參數，否則不會顯示。
